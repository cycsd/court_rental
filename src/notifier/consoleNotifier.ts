...
export function printConsoleSummary(result: TodayCheckResult): void {
  ...
  console.log(`Expired/Stop-rent Slots: ${result.expiredSlots}`);
    ...
    console.log("\n=== 各時段可用場地 ===");
    console.log("時間  | 可用數 | 可用場地");
    console.log("------+--------+------------------------------");
    for (const ts of result.timeSummary) {
        const ratio = `${ts.available}/${ts.total}`;
        const available = ts.availableCourts.length > 0 ? ts.availableCourts.join(", ") : "無可用場地";
    ...
    for (const court of result.courts) {
        ...
        for (const slot of courtSlots) {
            console.log(`  ${slot.time} | ${slot.isExpiredStopRent ? "Y" : "N"} | ${slot.rawStatus}`);
        }
  }
}
